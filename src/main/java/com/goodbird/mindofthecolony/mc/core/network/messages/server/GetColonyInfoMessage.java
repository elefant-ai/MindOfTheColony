...
import com.goodbird.mindofthecolony.mc.core.network.messages.client.OpenCantFoundColonyWarningMessage;
import com.goodbird.mindofthecolony.mc.core.network.messages.client.OpenColonyFoundingCovenantMessage;
import com.goodbird.mindofthecolony.mc.core.network.messages.client.OpenDeleteAbandonColonyMessage;
import com.goodbird.mindofthecolony.mc.core.network.messages.client.OpenReactivateColonyMessage;
...
import static com.goodbird.mindofthecolony.mc.api.util.constant.BuildingConstants.DEACTIVATED;
import static com.goodbird.mindofthecolony.mc.api.util.constant.TranslationConstants.HUT_BLOCK_MISSING_BUILDING;
...
public class GetColonyInfoMessage extends AbstractServerPlayMessage
{
    ...
    protected void onExecute(final IPayloadContext ctxIn, final ServerPlayer sender)
    {
        ...
        if (IColonyManager.getInstance().isFarEnoughFromColonies(world, pos))
        {
            ...
            {
                new OpenCantFoundColonyWarningMessage(Component.translatable("com.goodbird.mindofthecolony.mc.core.founding.tooclosetospawn", (int) (MineColonies.getConfig().getServer().minDistanceFromWorldSpawn.get() - spawnDistance)), pos, true).sendToPlayer(sender);
            }
            else if (spawnDistance > MineColonies.getConfig().getServer().maxDistanceFromWorldSpawn.get())
            {
                new OpenCantFoundColonyWarningMessage(Component.translatable("com.goodbird.mindofthecolony.mc.core.founding.toofarfromspawn", (int) (spawnDistance - MineColonies.getConfig().getServer().maxDistanceFromWorldSpawn.get())), pos, true).sendToPlayer(sender);
            }
        ...
        else
        {
            ...
            new OpenCantFoundColonyWarningMessage(Component.translatable("com.goodbird.mindofthecolony.mc.core.founding.tooclosetocolony", Math.max(100, blockRange - distance)), pos, false).sendToPlayer(sender);
        }
    }
}
