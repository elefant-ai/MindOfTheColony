...
import com.goodbird.mindofthecolony.mc.core.tileentities.TileEntityColonySign;
...
import static com.goodbird.mindofthecolony.mc.api.util.constant.TranslationConstants.NEXT;
import static com.goodbird.mindofthecolony.mc.api.util.constant.TranslationConstants.PREVIOUS;
import static com.goodbird.mindofthecolony.mc.core.blocks.BlockColonySign.CONNECTED;
...
public class TileEntityColonySignRenderer implements BlockEntityRenderer<TileEntityColonySign>
{
    ...
    private void renderTextOnSide(final PoseStack matrixStack, final float relativeRotationToColony, final @NotNull TileEntityColonySign tileEntity, final @NotNull MultiBufferSource buffer, final int combinedLight, final boolean mirrored)
    {
        ...
        if (colonyName.isEmpty())
        {
            ...
            renderText(matrixStack, buffer, combinedLight, Component.translatable("com.goodbird.mindofthecolony.mc.coremod.dist.blocks",distance).getString(), 3, 0);
        }
        else
        {
    ...
    private void renderColonyNameOnSign(final String colonyName, final PoseStack matrixStack, final @NotNull MultiBufferSource buffer, final int combinedLight, final int distance, final int offset)
    {
        ...
        if (textWidth > 90)
        {
            ...
            renderText(matrixStack, buffer, combinedLight, Component.translatable("com.goodbird.mindofthecolony.mc.coremod.dist.blocks",distance).getString(), 3, offset);
        }
        else
        {
