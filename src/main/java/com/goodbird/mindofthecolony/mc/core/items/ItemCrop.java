...
public class ItemCrop extends BlockItem
{
    ...
    protected boolean canPlace(BlockPlaceContext ctx, @NotNull BlockState state)
    {
        ...
        if (!player.isCreative())
        {
            ...
            {
                MessageUtils.format(Component.translatable("com.goodbird.mindofthecolony.mc.core.crop.cantplant")).sendTo(player);
            }
            return false;
        }
    ...
    public void appendHoverText(@NotNull final ItemStack stack, @Nullable final TooltipContext ctx, @NotNull final List<Component> tooltip, @NotNull final TooltipFlag flagIn)
    {
        ...
        if (preferredBiome != null && ctx.level() != null)
        {
            ...
            {
                tooltip.add(Component.translatable("com.goodbird.mindofthecolony.mc.core.item.crop.tooltip.biome.match").withStyle(ChatFormatting.GREEN));
            }
            else
            {
                tooltip.add(Component.translatable("com.goodbird.mindofthecolony.mc.core.item.crop.tooltip.biome.nomatch").withStyle(ChatFormatting.RED));
            }
        }
