...
import com.goodbird.mindofthecolony.mc.api.research.*;
import com.goodbird.mindofthecolony.mc.api.research.IResearchEffect;
import com.goodbird.mindofthecolony.mc.api.research.requirements.BuildingAlternatesResearchRequirement;
import com.goodbird.mindofthecolony.mc.api.research.requirements.BuildingResearchRequirement;
import com.goodbird.mindofthecolony.mc.api.research.util.ResearchState;
...
import com.goodbird.mindofthecolony.mc.core.client.gui.blockui.RotatingItemIcon;
import com.goodbird.mindofthecolony.mc.core.client.gui.modules.building.UniversityModuleWindow;
...
public class WindowResearchTree extends AbstractWindowSkeleton
{
    ...
    private void drawTreeBackground(final ZoomDragView view, final int maxHeight)
    {
        ...
        for (int i = 1; i <= MAX_DEPTH; i++)
        {
            ...
            {
                timeLabel.setText(Component.translatableEscape("com.goodbird.mindofthecolony.mc.coremod.gui.research.tier.header.unrestricted",
            ...
            {
                timeLabel.setText(Component.translatableEscape("com.goodbird.mindofthecolony.mc.coremod.gui.research.tier.header",
                  (i > building.getBuildingMaxLevel()) ? building.getBuildingMaxLevel() : i,
                  IGlobalResearchTree.getInstance().getBranchData(branch).getHoursTime(i)));
      ...
      final int progress)
    {
        ...
        switch (state)
        {
            ...
            case TOO_MANY_PROGRESS:
                ...
                tooMany1.setText(Component.translatableEscape("com.goodbird.mindofthecolony.mc.coremod.research.research.toomanyinprogress.1"));
                ...
                tooMany2.setText(Component.translatableEscape("com.goodbird.mindofthecolony.mc.coremod.research.research.toomanyinprogress.2"));
        ...
        if (state != ResearchButtonState.FINISHED && state != ResearchButtonState.IN_PROGRESS)
        {
            ...
            for (final SizedIngredient cost : research.getCostList())
            {
                hoverPaneBuilder.paragraphBreak()
                    ...
                    .append(Component.translatable("com.goodbird.mindofthecolony.mc.coremod.research.limit.cost", ItemStackUtils.getTranslatedName(cost)));
            ...
            {
                hoverPaneBuilder.paragraphBreak().append(Component.translatableEscape("com.goodbird.mindofthecolony.mc.coremod.research.requirement.university.level", Math.min(research.getDepth(), this.building.getBuildingMaxLevel())));
            }
            if (research.getDepth() == MAX_DEPTH && branchType != ResearchBranchType.UNLOCKABLES)
            {
                if (hasMax)
                {
                    hoverPaneBuilder.paragraphBreak().append(Component.translatableEscape("com.goodbird.mindofthecolony.mc.coremod.research.limit.onemaxperbranch")).color(COLOR_TEXT_UNFULFILLED);
                }
                else
                {
                    hoverPaneBuilder.paragraphBreak().append(Component.translatableEscape("com.goodbird.mindofthecolony.mc.coremod.research.limit.onemaxperbranch")).color(COLOR_TEXT_FULFILLED);
                }
            }
        }
        if (research.isImmutable())
        {
            hoverPaneBuilder.paragraphBreak().append(Component.translatableEscape("com.goodbird.mindofthecolony.mc.coremod.research.limit.immutable")).color(COLOR_TEXT_UNFULFILLED);
        }
    ...
    private void drawResearchTexts(final ZoomDragView view, final int offsetX, final int offsetY, final IGlobalResearch research, final ResearchButtonState state, final int progress)
    {
        ...
        if (state == ResearchButtonState.IN_PROGRESS)
        {
            ...
            progressLabel.setText(Component.translatableEscape("com.goodbird.mindofthecolony.mc.coremod.gui.research.time", timeRemaining));
    ...
    private void drawUndoProgressButton(final Button parent)
    {
        ...
        undoText.setText(Component.translatableEscape("com.goodbird.mindofthecolony.mc.coremod.research.undo.progress"));
        undoText.disable();
        parent.getParent().addChild(undoText);
        PaneBuilders.tooltipBuilder().hoverPane(undoButton).append(Component.translatableEscape("com.goodbird.mindofthecolony.mc.coremod.research.undo.progress.tooltip")).color(COLOR_TEXT_UNFULFILLED).bold().build();
    }
    ...
    private void drawUndoCompleteButton(final Button parent)
    {
        ...
        final AbstractTextBuilder.TooltipBuilder undoTipBuilder = PaneBuilders.tooltipBuilder().hoverPane(undoButton)
                                  .append(Component.translatableEscape("com.goodbird.mindofthecolony.mc.coremod.research.undo.remove.tooltip")).bold().color(COLOR_TEXT_UNFULFILLED);
        undoText.setSize(BUTTON_LENGTH, BUTTON_HEIGHT);
        ...
        if (!missingItems.isEmpty())
        {
            ...
            undoText.setText(Component.translatableEscape("com.goodbird.mindofthecolony.mc.coremod.research.research.notenoughresources"));
            for (ItemStorage cost : missingItems)
            {
                undoTipBuilder.paragraphBreak().append(Component.translatableEscape("com.goodbird.mindofthecolony.mc.coremod.research.requirement.research",
        ...
        else
        {
            undoButton.setImage(new ResourceLocation(Constants.MOD_ID, MEDIUM_SIZED_BUTTON_RES));
            undoButton.setID("undo:" + parent.getID());
            undoText.setText(Component.translatableEscape("com.goodbird.mindofthecolony.mc.coremod.research.undo.remove"));
        }
        undoText.disable();
        parent.getParent().addChild(undoButton);
        parent.getParent().addChild(undoText);
      ...
      final ResearchButtonState state)
    {
        if (research.isImmutable() && state != ResearchButtonState.FINISHED)
        {
            ...
            PaneBuilders.tooltipBuilder().hoverPane(immutIcon).paragraphBreak().append(Component.translatableEscape("com.goodbird.mindofthecolony.mc.coremod.research.limit.immutable"))
        ...
        else
        {
            ...
            else
            {
                if (IGlobalResearchTree.getInstance().getResearch(branch, parentResearch).hasOnlyChild())
                {
                    ...
                    orLabel.setText(Component.translatableEscape("com.goodbird.mindofthecolony.mc.coremod.research.research.or"));
                    orLabel.setPosition(offsetX + INITIAL_X_OFFSET, offsetY + TEXT_Y_OFFSET);
                    view.addChild(orLabel);
                    PaneBuilders.tooltipBuilder().hoverPane(orLabel).append(Component.translatable("com.goodbird.mindofthecolony.mc.coremod.research.research.or.tooltip")).build();
