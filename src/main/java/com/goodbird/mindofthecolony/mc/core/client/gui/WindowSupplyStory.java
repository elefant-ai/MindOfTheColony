...
import com.goodbird.mindofthecolony.mc.api.items.component.SupplyData;
import com.goodbird.mindofthecolony.mc.api.util.constant.Constants;
import com.goodbird.mindofthecolony.mc.core.event.ColonyStoryListener;
import com.goodbird.mindofthecolony.mc.core.network.messages.server.MarkStoryReadOnItem;
...
import static com.goodbird.mindofthecolony.mc.api.items.ISupplyItem.SUPPLY_OFFSET_DISTANCE;
...
public class WindowSupplyStory extends AbstractWindowSkeleton
{
    ...
    public WindowSupplyStory(final BlockPos pos, final String type, final ItemStack stack, final InteractionHand hand)
    {
        ...
        story.add(Component.translatable("com.goodbird.mindofthecolony.mc.core.gui.supplies.guide", Component.translatable(stack.getItem().getDescriptionId())));
        story.add(Component.empty());

        if (stack.getItem() == ModItems.supplyCamp)
        {
            story.add(Component.translatable("com.goodbird.mindofthecolony.mc.core.gui.supplycamp.guide"));
        }
        else
        {
            story.add(Component.translatable("com.goodbird.mindofthecolony.mc.core.gui.supplyship.guide"));
        }

        this.findPaneOfTypeByID("text", Text.class).setText(story);
        this.findPaneOfTypeByID("place", Button.class).setText(Component.translatable("com.goodbird.mindofthecolony.mc.core.gui.supplies.place", Component.translatable(stack.getItem().getDescriptionId())));
    }
