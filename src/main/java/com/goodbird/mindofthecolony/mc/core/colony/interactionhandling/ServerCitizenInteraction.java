...
import com.goodbird.mindofthecolony.mc.api.colony.interactionhandling.AbstractInteractionResponseHandler;
...
import com.goodbird.mindofthecolony.mc.core.client.gui.citizen.MainWindowCitizen;
import com.goodbird.mindofthecolony.mc.core.network.messages.server.colony.InteractionResponse;
...
public abstract class ServerCitizenInteraction extends AbstractInteractionResponseHandler
{
    ...
    public boolean onClientResponseTriggered(final int responseId, final Player player, final ICitizenDataView data, final BOWindow window)
    {
        final Component response = getPossibleResponses().get(responseId);
        tryHandleIgnoreResponse(response, player);
        if (((TranslatableContents) getPossibleResponses().get(responseId).getContents()).getKey().equals("com.goodbird.mindofthecolony.mc.coremod.gui.chat.skipchitchat"))
        {
